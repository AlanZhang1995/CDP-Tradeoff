#DNCNN structure  + Add noise on line + select best G_loss model + MSE decay + Tensorboard + weight3*0.1
import os, sys
...
parser.add_argument('--sigma', type=int, default=1, help='standard deviation of niose')
parser.add_argument('--weight1', type=float, default=1, help='loss weight for MSE')
parser.add_argument('--weight2', type=float, default=0.3, help='loss weight for GAN')
parser.add_argument('--weight3', type=float, default=1, help='loss weight for Classifer')
parser.add_argument('--pretrained', type=str, default=None, help='checkpoint dir for pretrained denoiser')
parser.add_argument('--outdir', type=str, default='/media/alan/SteinsGate/github_code/mnist/TensorFlow_WGAN/improved_wgan_training-master-sort/samples/mnist', help='Dir for saving samples')
args = parser.parse_args()
if not os.path.isdir(args.outdir):
    os.makedirs(args.outdir)
    os.makedirs(args.outdir+'/logs/')
inputfile='Dateset_NoisyMnist_{}'.format(args.sigma)+'.npz'
...
print('Standard deviation of niose is {}'.format(args.sigma))
...
def Generator(n_samples, noise=None, ADDNoise=1):
    if ADDNoise==1:
        noise = noise + tf.random_normal([n_samples, OUTPUT_DIM],stddev=args.sigma)
    _input = tf.reshape(noise, [-1, 1, 28, 28])
    #output = lib.ops.linear.Linear('Generator.Input', 784, 4*4*4*DIM, noise)
    output = lib.ops.conv2d.Conv2D('Generator_conv1.0',1, DIM, 5, _input)
    output = tf.nn.relu(output)
    output = lib.ops.conv2d.Conv2D('Generator_conv1.1',DIM, 2*DIM, 5, output)
    output = lib.ops.batchnorm.Batchnorm('Generator_conv1.BN1', [0,2,3], output)
    output = tf.nn.relu(output)
    output = lib.ops.conv2d.Conv2D('Generator_conv1.2',2*DIM, 2*DIM, 5,output)
    output = lib.ops.batchnorm.Batchnorm('Generator_conv1.BN2', [0,2,3], output)
    output = tf.nn.relu(output)
    output = lib.ops.conv2d.Conv2D('Generator_conv1.3',2*DIM, DIM, 5,output)
    output = lib.ops.batchnorm.Batchnorm('Generator_conv1.BN3', [0,2,3], output)
    output = tf.nn.relu(output)
    output = lib.ops.conv2d.Conv2D('Generator_conv1.4',DIM, 1, 5,output)
...
#test--build
fixed_noise_samples = Generator(10000, noise=Tinput_noise, ADDNoise=0)
